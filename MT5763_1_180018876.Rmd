...
hour_avg_plot <- hour_count_average %>%
  ...
  ggtitle("Time of Day vs Bike Rental") +
  ylab("Number of bikes hired per hour")
hour_avg_plot
```
There's clearly an increase at commuting times of 9 and 16-18 hours with a sharp decrease in the late hours of the night. 


## Plotting Weather versus Bike Usage
Here we will Plot to see the relationship between the different meteorlogoical variables and the number of bikes rented. 
### Temperature
...
air_temp_plot
```
While we can see there is clearly a positive linear relationship between bike rentals and temperature, there is clearly way too many datapoints for us to interpret this data properly in this format. In order to clean it up, we will instead plot the average bike count for each temperature. In order to accomplish this, first I'll need to compute the average:
...
### Wind v Bike Demand
Again we will group the average the Counts by Windspeed:
...
wind_plot
```
For both of these plots, it looks quite clearly non-linear with a peak of around 2.5(m/s) for Seoul and 5(m/s). This could be due to the fact that it is either non-linear 
